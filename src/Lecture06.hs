...
{-
  ...
  Докажем "от противного". Пусть существует такой контекст.
  Замечу, что x x : T => x - аппликация => x : T' -> T.
  Т.к x принимает себя в качестве аргумента, то x : T'.
  Получается, что T' !== T' -> T => противоречие.
-}
-- </Задачи для самостоятельного решения>
