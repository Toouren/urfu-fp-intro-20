...
createTodoList path = do
  createDirectory path
  return $ TodoList path

addTodo :: TodoList -> Title -> Content -> Deadline -> IO Id
addTodo (TodoList path) title content deadline = do
  (fileName, file) <- openTempFile path "" 
  let
...
getTodoFilePath (TodoList path) (Id fileName) = joinPath [path, fileName]
...
replaceTodo todoList@(TodoList path) todoId todo = do
  (tempName, tempFile) <- openTempFile path "temp"
...
main secret = do
  ...
  case (compare (read guess) secret) of
    LT -> putStrLn "Too small" >> main secret
    GT -> putStrLn "Too big" >> main secret
